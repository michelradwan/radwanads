...
async function runAudit() {
    ...
    await serverState.releaseLock('act_846780837970771');
    const lock1 = await serverState.acquireLock('act_846780837970771', 60);
    const lock2 = await serverState.acquireLock('act_846780837970771', 60);
    ...
    const idempBefore = await serverState.checkIdempotency(testActionId);
    assert.strictEqual(idempBefore.isDuplicate, false, 'Primeira execução não deve ser duplicada');

    await serverState.recordIdempotency(testActionId, { success: true, budget: 6000 });
    const idempAfter = await serverState.checkIdempotency(testActionId);
    ...
    assert.strictEqual(testAllowlist('invalid_endpoint_xyz/123/evil', 'GET'), false, 'Endpoint inválido ou malformado deve ser BLOQUEADO');
    ...
    const missingInsight = { spend: '100', clicks: '50' };
    ...
    await serverState.setUnitEconomicsVerified(false);
    assert.strictEqual(await serverState.isUnitEconomicsVerified(), false, 'Unit Economics não verificado deve bloquear escala');
    await serverState.setUnitEconomicsVerified(true);
    assert.strictEqual(await serverState.isUnitEconomicsVerified(), true, 'Unit Economics verificado libera escala');
    await serverState.setUnitEconomicsVerified(false);
