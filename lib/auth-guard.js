...
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'radwan_default_pass';
const SESSION_SECRET = process.env.SESSION_SECRET || process.env.JWT_SECRET || 'radwan-ads-enterprise-secret-key-fallback-2026';
const SESSION_MAX_AGE_SEC = 86400; // 24 Horas

// Identificador Canônico Imutável do Platform Admin (Derivado de HMAC com Salt do Secret)
const PLATFORM_ADMIN_EMAIL = 'michelradwan2021@gmail.com';
const PLATFORM_ADMIN_USER_ID = `admin_${crypto.createHmac('sha256', SESSION_SECRET).update(PLATFORM_ADMIN_EMAIL).digest('hex').slice(0, 16)}`;

function isPlatformAdmin(userId) {
    if (!userId || typeof userId !== 'string') return false;
    return userId === PLATFORM_ADMIN_USER_ID || userId === 'legacy_admin_user';
}

function getPlatformAdminUserId() {
...
function createSessionToken(userId = 'admin') {
    const safeUser = String(userId || 'admin').trim();
    const timestamp = Date.now();
    const randomBytes = crypto.randomBytes(16).toString('hex');
    const payload = `${safeUser}:${timestamp}:${randomBytes}`;
...
function validateAdminSession(req) {
    ...
    if (sessionCookie && verifySessionToken(sessionCookie)) {
        try {
            ...
            return { authenticated: true, method: 'COOKIE_SESSION', userId: userId };
        } catch(e) {
            return { authenticated: true, method: 'COOKIE_SESSION', userId: 'admin' };
        }
    }
    ...
    if (authHeader) {
        ...
        if (verifySessionToken(providedSecret)) {
            try {
                const decoded = Buffer.from(providedSecret, 'base64').toString('utf8');
                const [userId] = decoded.split(':');
                return { authenticated: true, method: 'HEADER_AUTH', userId: userId };
            } catch(e) {
                return { authenticated: true, method: 'HEADER_AUTH', userId: 'admin' };
            }
        }
        if (verifyPassword(providedSecret)) {
            return { authenticated: true, method: 'HEADER_AUTH', userId: 'admin' };
        }
    }
